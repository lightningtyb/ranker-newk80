...
def read_eval_trec(file):
    ...
    for i, line in enumerate(tqdm(open(file, 'r'), desc='loading run (by line)', leave=False)):
        cols = line.split()
        if len(cols) == 6:  # for doc ranking
            qid, _, docid, rank, score, _ = cols
            result.setdefault(qid, {})[docid] = 0

        elif len(cols) == 2:  # for passage ranking
            qid, docid = line.rstrip().split('\t')
            result.setdefault(qid, {})[docid] = 0

        else: # qrel for debug rerank
            qid, _, docid, label = line.split()
...
def read_train_pairs(file, queries, docs, qrel):
    ...
    for line in tqdm(open(file, 'r'), desc='loading pairs (by line)', leave=False):
        ...
        if len(cols) == 2:
            ...
            qid, pos_id, neg_id = cols
            if qid not in qrel or qid not in queries or pos_id not in docs or neg_id not in docs:
                continue
            qids.add(qid)
            qd_pairs.setdefault(qid, {})[pos_id] = 1
            qd_pairs.setdefault(qid, {})[neg_id] = 0
            pos_num += 1
    # print('qd_pairs:',qd_pairs)
...
def convert_example_to_features(query_toks, doc_toks, label, max_seq_length, tokenizer):
    ...
    inputs = tokenizer.prepare_for_model(tokens_b_ids,tokens_a_ids,add_special_tokens=True,truncation='only_first',max_length=max_seq_length,return_token_type_ids=True)
...
def convert_example2features_bart_encodeer(query_toks, doc_toks, label, max_seq_length, tokenizer):
    tokens_a_ids = tokenizer.convert_tokens_to_ids(query_toks)
    tokens_b_ids = tokenizer.convert_tokens_to_ids(doc_toks)
...
DecoderInputFeatures = namedtuple("DecoderInputFeatures", "input_ids input_mask segment_ids query_len query_mask label ")

def convert_example2features_bart_decoder(query_toks, doc_toks, label, max_seq_length, tokenizer):

    tokens_a_ids = tokenizer.convert_tokens_to_ids(query_toks)
    tokens_b_ids = tokenizer.convert_tokens_to_ids(doc_toks)
    query_len = len(tokens_a_ids)

    inputs = tokenizer.prepare_for_model(tokens_b_ids,tokens_a_ids,add_special_tokens=True,padding=True,truncation='only_first',max_length=max_seq_length,return_token_type_ids=True)
    input_ids = inputs['input_ids']
    input_ids[len(tokens_b_ids)+2:] = input_ids[len(tokens_b_ids)+3:]

    segment_ids = inputs['token_type_ids']
    segment_ids[len(tokens_b_ids)+2:] = segment_ids[len(tokens_b_ids)+3:]
    ...
    mask_array = np.zeros(max_seq_length, dtype=np.int)
    mask_array[:len(input_ids)] = 1  # 有值为1，是input_dis的长度，空的为0

    segment_array = np.zeros(max_seq_length, dtype=np.int)
    segment_array[:len(segment_ids)] = segment_ids

    query_mask_array = np.zeros(max_seq_length, dtype=np.int)
    query_mask_array[len(tokens_b_ids) + 2:len(tokens_a_ids) + len(tokens_b_ids) + 3] = 1

    features = DecoderInputFeatures(input_ids=input_array,
                                    input_mask=mask_array,
                                    segment_ids=segment_array,
                                    query_len=query_len,
                                    query_mask=query_mask_array,
                                    label=label,)
    return features
    ...
    def __init__(self, args, queries, docs, train_pairs, qrels, epoch, tokenizer, mode='train'):
        ...
        for i, example in enumerate(tqdm(examples, desc=f"epoch: {epoch}, Converting {mode} examples to features.", leave=False)): # total=num_samples, 
            ...
            if instance_index+1 > num_samples:
                break
            if args.model_name == 'bart_decoder':
                features = convert_example2features_bart_decoder(queries[qid], docs[did], label, self.seq_len, tokenizer)
            elif args.model_name == 'bart_encoder':
                features = convert_example2features_bart_encodeer(queries[qid], docs[did], label, self.seq_len, tokenizer)
            else:
                features = convert_example_to_features(queries[qid], docs[did], label, self.seq_len, tokenizer)
            labels[instance_index] = features.label # 表示样本正负
...
class PregeneratedDatasetForDecoder(Dataset):
    def __init__(self, args, queries, docs, train_pairs, qrels, epoch, tokenizer, mode='train'):
        self.epoch = epoch
        self.working_dir = None
        self.tokenizer = tokenizer
        ...
        labels = np.memmap(filename=self.working_dir / 'labels.memmap',
                           shape=(num_samples), mode='w+', dtype=np.bool)
        query_lens = np.memmap(filename=self.working_dir / 'query_lens.memmap',
                               shape=(num_samples), mode='w+', dtype=np.int32)
        query_masks = np.memmap(filename=self.working_dir / 'query_masks.memmap',
                                shape=(num_samples, self.seq_len), mode='w+', dtype=np.int32)

        instance_index = 0
        for i, example in enumerate(tqdm(examples, desc=f"epoch: {epoch}, Converting {mode} examples to features.",
                                         leave=False)):  # total=num_samples,
            qid, did, label = example
            if instance_index + 1 > num_samples:
                break
            features = convert_example2features_bart_decoder(queries[qid], docs[did], label, self.seq_len,
                                              tokenizer)
            labels[instance_index] = features.label  # 表示样本正负
            input_ids[instance_index] = features.input_ids
            input_ids[instance_index] = features.input_ids
            segment_ids[instance_index] = features.segment_ids
            input_masks[instance_index] = features.input_mask
            query_lens[instance_index] = features.query_len
            query_masks[instance_index] = features.query_mask
            instance_index += 1
    ...
    def __getitem__(self, item):
        return (torch.tensor(self.input_ids[item].astype(np.int64)),
                torch.tensor(self.input_masks[item].astype(np.int64)),
                torch.tensor(self.segment_ids[item].astype(np.int64)),
                torch.tensor(int(self.query_lens[item])),
                torch.tensor(self.query_masks[item].astype(np.int64)),
                torch.tensor(int(self.labels[item]))
                )

class PregeneratedPositiveDatasetForDecoder(Dataset):
    def __init__(self, args, queries, docs, train_pairs, qrels, epoch, tokenizer, mode='train'):
        self.epoch = epoch
        self.working_dir = None
        self.tokenizer = tokenizer
        ...
        if mode == 'train':
            ...
            for qid in tqdm(qids):
                ...
                pos_id = np.random.choice(pos_ids)
                examples.append((qid, pos_id, qrels[qid][pos_id]))
                num_samples += len(pos_ids)
        assert num_samples != 0  # Assert that the sample count metric was true
        args.pos_num = num_samples

        logging.info('Positive samples for epoch-{}: {}'.format(epoch, num_samples))
...
def iter_valid_records(args, model, queries, docs, run, tokenizer):
    batch = {'query_id': [], 'doc_id': [], 'input_ids': [], 'segment_ids': [], 'input_mask':[],'query_len':[],'query_mask':[], 'label': []}
    for qid, did in _iter_valid_records(queries, docs, run):
        if args.model_name == 'bart_decoder':
            features = convert_example2features_bart_decoder(queries[qid], docs[did], 0, args.max_sequence_length, tokenizer)
        elif args.model_name == 'bart_encoder':
            features = convert_example2features_bart_decoder(queries[qid], docs[did], 0, args.max_sequence_length, tokenizer)
        else:
        ...
        batch['query_len'].append(features.query_len)
        batch['query_mask'].append(features.query_mask)
        ...
        if len(batch['query_id']) == args.eval_batch_size:
            ...
            batch['query_len'] = torch.tensor(batch['query_len']).long()
            batch['query_mask'] = torch.tensor(batch['query_mask']).float()
    ...
    if len(batch['query_id']) > 0:
        ...
        batch['input_mask'] = torch.tensor(batch['input_mask']).float()
        batch['query_len'] = torch.tensor(batch['query_len']).long()
        batch['query_mask'] = torch.tensor(batch['query_mask']).long()
        batch['label'] = torch.tensor(batch['label']).long()
        yield batch
