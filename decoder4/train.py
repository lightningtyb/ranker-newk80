...
from modeling import BertRanker, RobertaRanker, BartEncoderRanker
from model import BartDecoderRanker
...
class RandomPairSampler(torch.utils.data.Sampler):
    def __init__(self, data_source, negtive=1):
        ...
        if (len(self.data_source) % (self.negtive + 1)) != 0:
            raise ValueError(
                'data length {} % {} !=0, can not pair data!'.format(len(self.data_source), self.negtive + 1))
...
def train(args, model, queries, docs, qrel, train_pairs, valid_run):
    ...
    if args.warm_up > 0:
        if args.strategy == 'q':
            num_train_optimization_steps = math.ceil(args.epochs * (
                        args.negative_num + 1) * args.queries_per_epoch / args.train_batch_size / args.gradient_accumulation_steps)
        elif args.strategy == 'd':
            num_train_optimization_steps = math.ceil(args.epochs * (
                        args.negative_num + 1) * args.pos_num / args.train_batch_size / args.gradient_accumulation_steps)
        ...
        logging.info('{} lr:{}'.format(args.model_name,args.learning_rate))
    else:
        # no warmup
        LR = args.learning_rate
        params = [(k, v) for k, v in model.named_parameters() if v.requires_grad]
        non_bert_params = {'params': [v for k, v in params if not k.startswith('bart.')]}
        bert_params = {'params': [v for k, v in params if k.startswith('bart.')], 'lr': args.learning_rate}
    ...
    for epoch in range(args.epochs):
        ...
        epoch_train_dataset = PregeneratedDataset(args, queries, docs, train_pairs, qrel, epoch, tokenizer,
                                                  mode='train')
        ...
        loss = train_epoch(args, model, train_dataloader, queries, docs, valid_run, optimizer, scheduler, epoch, device,
                           n_gpu)
        ...
        if args.eval_after_epoch > 0 and (epoch + 1) % args.eval_after_epoch == 0:
            ...
            if best_eval_score is None or eval_score > best_eval_score:
                ...
                logging.info('new top validation score {} at epoch-{}, step-{}, saving weights'.format(best_eval_score,
                                                                                                       best_eval_epoch,
                                                                                                       best_eval_step))
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if args.eval_interval_steps != -1 and global_step % args.eval_interval_steps == 0:
                ...
                if best_eval_score is None or eval_score > best_eval_score:
                    ...
                    best_eval_step = global_step
                    best_eval_score = eval_score
                    logging.info('new top validation score {} at step-{}, saving weights'.format(best_eval_score,
                                                                                                 best_eval_step))
                    model.module.save(temp_output_dir) if hasattr(model, 'module') else model.save(temp_output_dir)
...
def msamrco_eval(qrelf, runf):
    ...
    eval_f = '/home/tangyubao/bert-reranker-main/bin/eval_msmarco.sh'
...
def main():
    ...
    parser.add_argument('--patience', default=300, type=int,
                        help='Stop training when performance do not improve after several steps.')
    ...
    parser.add_argument('--eval_interval_steps', default=-1, type=int,
                        help='Validation after several steps, -1 default is not do eval.')
    ...
    parser.add_argument('--data_dir', type=Path, default=None,
                        help='The data dir consists of query file, doc file and the qrel file')
    ...
    parser.add_argument('--temp_dir', required=True,
                        help='the temporary directory where the document data will be saved')
    ...
    if args.model_name == 'bert':
        ...
        MODEL = BartEncoderRanker
    elif args.model_name == 'bart_decoder':
        MODEL = BartDecoderRanker
    else:
        MODEL = RobertaRanker

    # MODEL = BertRanker if args.model_name == 'bert' else RobertaRanker
    model = MODEL(args.model_path).cuda()
    if model == 'bart_decoder':
        tokenizer = model.get_tokenizer(args.model_path)
    else:
        tokenizer = model.get_tokenizer()

    if args.data_dir:
        queries, docs, qrel = utils.read_data_from_dir(args.data_dir, args.temp_dir, args.max_sequence_length,
                                                       tokenize=args.tokenize_doc)
    else:
        ...
        docs = utils.read_docfile(args.temp_dir, args.doc_file, args.max_sequence_length,
                                  tokenize_doc=args.tokenize_doc, tokenizer=tokenizer)
