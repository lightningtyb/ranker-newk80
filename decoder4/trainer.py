...
from transformers import BartTokenizer, WEIGHTS_NAME, CONFIG_NAME
...
import utils
from utils import PregeneratedDataset, PregeneratedDatasetForDecoder,PregeneratedPositiveDatasetForDecoder
from modeling import BertRanker, BartEncoderRanker, BartDecoderRanker
...
def train(args, model, queries, docs, qrel, train_pairs, valid_run, tokenizer):
    ...
    if args.warm_up > 0:
        ...
        logging.info('model lr:{}'.format(args.learning_rate))
    else:
        # no warmup
        LR = args.learning_rate
        params = [(k, v) for k, v in model.named_parameters() if v.requires_grad]
        non_bert_params = {'params': [v for k, v in params if not k.startswith('decoder.')]}
        bert_params = {'params': [v for k, v in params if k.startswith('decoder.')], 'lr': args.learning_rate}
    ...
    for epoch in range(args.epochs):
        ...
        if args.model_name == 'bart_decoder':
            epoch_train_dataset = PregeneratedDatasetForDecoder(args, queries, docs,train_pairs, qrel, epoch, tokenizer,mode='train')
        else:
        ...
        loss = train_epoch(args, model, train_dataloader, queries, docs, valid_run, optimizer, scheduler, epoch, device, n_gpu,tokenizer)
        logging.info(f'train epoch={epoch} loss={loss}')

        if args.eval_after_epoch > 0 and (epoch+1) % args.eval_after_epoch == 0:
            eval_score = validate(args, model, queries, docs, valid_run, args.qrel_file, epoch, device, tokenizer)
            ...
            if best_eval_score is None or eval_score > best_eval_score:
                ...
                save(temp_output_dir, model, tokenizer)
...
def save(temp_output_dir, model, tokenizer):
...
def train_epoch(args, model, train_dataloader, queries, docs, valid_run, optimizer, scheduler, epoch, device, n_gpu,tokenizer):
    ...
    with tqdm(total=len(train_dataloader), desc=f"Epoch {epoch}", leave=True) as pbar:
        for i, batch in enumerate(train_dataloader):
            ...
            input_ids, input_mask, segment_ids, query_lens, query_masks, labels = batch
            # if args.model_name == 'bart_decoder' and args.loss_type == 'ce':
            #     ranking_loss, mse_loss = model(input_ids, segment_ids, input_mask, query_lens, query_masks, device, args.loss_type,
            #                                    labels, args.negative_num + 1)
            #     args.writer.add_scalars('train', {'ranking_loss': ranking_loss.item()}, global_step)
            #     args.writer.add_scalars('train', {'mse_loss': mse_loss.item()}, global_step)
            #     loss = ranking_loss + mse_loss
            # else:
            loss = model(input_ids, segment_ids, input_mask,  query_lens, query_masks,device,args.loss_type, labels, args.negative_num + 1)
            ...
            if args.eval_interval_steps != -1 and global_step % args.eval_interval_steps == 0:
                eval_score = validate(args, model, queries, docs, valid_run, args.qrel_file, global_step, device,tokenizer)
...
def validate(args, model, queries, docs, eval_run, qrelf, global_step, device,tokenizer):
    result = None
    runf = os.path.join(args.model_out_dir, f'{global_step}.run')
    run_eval(args, model, queries, docs, eval_run, runf, device, tokenizer)
    if args.task == 'mq2007' or args.task == 'mq2008':
        ...
        result = msamrco_eval(qrelf, runf)
    else:
...
def run_eval(args, model, queries, docs, run, runf, device,tokenizer, desc='valid'):
    rerank_run = {}
    model.eval()
    with torch.no_grad(), tqdm(total=sum(len(r) for r in run.values()), ncols=80, desc=desc, leave=False) as pbar:
        for records in utils.iter_valid_records(args, model, queries, docs, run, tokenizer):
            if args.model_name == 'bart_decoder':
                scores, logits = model(records['input_ids'].to(device),
                                       records['segment_ids'].to(device),
                                       records['input_mask'].to(device),
                                       records['query_len'].to(device),
                                       records['query_mask'].to(device), device, None,None)
            else:
...
def trec_eval(qrelf, runf, metric):
    trec_eval_f = '/users/tangyubao/ranker-newk80/bin/trec_eval'
...
def mq_eval(fold_num, runf, metric, task):
    eval_f = '/users/tangyubao/ranker-newk80/bin/eval_mq.sh'
...
def main():
    ...
    if args.model_name == 'bart_encoder':
        model = BartEncoderRanker(config).from_pretrained(args.model_path).cuda()
    elif args.model_name == 'bart_decoder':
        model = BartDecoderRanker(config).from_pretrained(args.model_path).cuda()
    ...
    if args.data_dir:
        ...
        queries = utils.read_queryfile(args.query_file, tokenizer)
        queries = utils.read_queryfile(args.query_file, tokenizer)

        docs = utils.read_docfile(args.temp_dir, args.doc_file, args.max_sequence_length, tokenize_doc=args.tokenize_doc, tokenizer=tokenizer)
    ...
    train(args, model, queries, docs, qrel, train_pairs, valid_run, tokenizer)
